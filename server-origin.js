...
let	deleteRepeat = {};	//去重哈希数组
let	urlsArray = [];	//存放爬取网址
let	catchData = [];	//存放爬取数据
let	pageUrls = [];	//存放收集文章页面网站
const	pageNum = 2;	//要爬取文章的页数
const	startDate = new Date();	//开始时间
const	endDate = false;	//结束时间

const	catchFirstUrl = 'http://www.cnblogs.com/';	//入口页面
...
function start(){
	function onRequest(req, res){
		...
		// 当所有 'BlogArticleHtml' 事件（即监听到pageUrls.length*20次该事件后）完成后的回调触发下面事件。
		// articleUrls是个数组，每一项是该事件携带的数据:articleUrl。
      ...
      async.mapLimit(articleUrls, 5, 
      	function (url, callback) {
				...
				function (err,result) {
				  ...
				  for(var i=0 ; i<len ; i++){
				  	var eachDate = JSON.stringify(catchData[i]),
				  		eachDateJson = catchData[i];
